        ...
        if (userSnapshot.exists()) {
          ...
          try {
            profilePicUrl = await getDownloadURL(
              storageRef(storage, `profilePictures/${auth.currentUser.uid}`)
            );
          } catch (error) {
        ...
        const connectionList = await Promise.all(
          snapshot.docs.map(async (connectionDoc) => {
            ...
            const fromUserName = userSnapshot.exists()
              ...
              : connectionData.from; // Fallback to UID if name is unavailable
    ...
    <div className="new-dashboard-container">
      ...
      <div className="sidebar">
        ...
        <ul className="nav-links">
          ...
          <li
            className={currentView === "mentorHub" ? "active" : ""}
            onClick={() => setCurrentView("mentorHub")}
          >
            <FaChalkboardTeacher /> Mentor Hub
          </li>
          <li
            className={currentView === "investorHub" ? "active" : ""}
            onClick={() => setCurrentView("investorHub")}
          >
          ...
          <li
            className={currentView === "healthDashboard" ? "active" : ""}
            onClick={() => setCurrentView("healthDashboard")}
          >
            <FaChartLine /> Business Health
          </li>
          <li
            className={currentView === "aiChatbot" ? "active" : ""}
            onClick={() => setCurrentView("aiChatbot")}
          >
            <FaRobot /> AI Chatbot
          </li>
          <li
            className={currentView === "apolloConnections" ? "active" : ""}
            onClick={() => setCurrentView("apolloConnections")}
          >
            <FaSearch /> Apollo Connections
          </li>
          <li
            className={currentView === "thinkTank" ? "active" : ""}
            onClick={() => setCurrentView("thinkTank")}
          >
          ...
          >
            ...
            {newReceivedRequests > 0 && (
              <span className="badge">{newReceivedRequests}</span>
            )}
          </li>
        </ul>
      </div>
                    ...
                    <ul>
                      {connections.map((connection) => (
                        <li key={connection.id}>
                          Connected with {connection.fromUserName}
                        </li>
                      ))}
                    </ul>
                  </div>
      ...
      {isEditProfileOpen && (
        <div className="edit-profile-modal">
          <div className="modal-header">
            <h2>Edit Profile</h2>
            <button className="close-button" onClick={toggleEditProfileModal}>
              X
            </button>
          </div>
